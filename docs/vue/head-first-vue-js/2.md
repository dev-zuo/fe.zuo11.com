...
完整 demo：[index-array | my-vue | vue2-implement](https://github.com/zuoxiaobai/vue2-implement/blob/vue2/my-vue/index-array.html)
...
- 4、在 Observer 中，对每个侦测了变化的数据都增加了 `__obj__`, 并把 this （Observer）实例保存在 `__ob__` 上，有两个作用 1、标记数组是否已经做了侦测，只被侦测一次。2、可以很方便的通过数据的 `__obj__` 拿到 Observer 上保存的依赖，当拦截到数组变化时，向依赖发送通知。
