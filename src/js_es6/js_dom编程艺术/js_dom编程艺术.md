...
```
### 结构化程序设计
（page79）尽量将通用的方法抽象出来，避免重复复调用，一个函数应该只有一个入口和出口。如果一个函数有多个出口，如return false，只要这些出口，集中在开头是可以接受的
```js
...
```
### onload事件
(page83) 由于DOM的获取需要DOM先渲染完成，如果把js文件导入放到head里，没有加上async、defer或者没有将js放在body末尾子节点，会先执行js，阻塞DOM渲染，如果js里面有获取dom的操作会获取不到DOM元素，onload事件即在页面load后，执行，可以确保dom以渲染完成
```js
