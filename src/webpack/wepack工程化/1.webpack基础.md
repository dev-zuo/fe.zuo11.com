...
```
而且，我们修改 src/index.js 文件后，可以自动重新rebuild，刷新页面。注意：这里修改src/index.js里面引入的文件，不会触发rebuild.

**代理服务器：解决mock接口连后端本地接口跨域的问题**
...
// && 是按顺序执行 concurrently 是并列执行，跨平台的并列执行，可以使用 npm-run-all
