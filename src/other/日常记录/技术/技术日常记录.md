...
### 2020/06/17 周三
#### v-loading 指令的实现
#### v-loading可以自动加loading，怎么通过一个指令自动加骨架屏呢？
可以在某个区域内加loading，那么 v-loading2 可以在某个区域内填充骨架屏吗？
...
Last-Modify传给后端，如果后端根据这个时间来看是否304，让客户端自己读缓存，

### 2020/06/15 周一
#### macos大小写不敏感，git会错乱
macos是文件大小写不敏感的系统，而widonws/linux是大小写敏感的

macos是大小写不敏感的文件系统，也就是在macos上你无法在同一个目录下同时创建test.vue和Test.vue，如果出现这种情况，最好的方法是将整个文件名都变更。
#### echarts画高自定义仪表盘，echart本质是图的堆叠
#### 输入过程中，怎么实时高亮部分文字(@xxx高亮实现)
#### 怎么将mp4和m4a文件合并
#### element loading在IE下可能会溢出到全屏的问题
#### eleemnt loading在IE下可能会关不掉的问题
...
主要是安装了babel，babel有进行转换，当IE不支持某个方法时，会使用替代的pollyfill，那它是怎么打包进项目的，写个hello world测试下原理看看

#### mock接口时router.allowedMethods()这个中间间是用来做什么的？
```js
app.use(KoaBody()).app.use(router.routes()).use(router.allowedMethods())
```
为什么要加最后一个方法呢？

一般情况下，假设你定义了 router.get('/user')这个方法，而恰好用post请求了这个方法，会报404，接口没找到

如果你加了这个中间件，上面这种情况就会返回405, method not allowed，而且请求头Allow还会给出支持的method。

来看下源码：


### 2020/06/12 周五
#### component中的组件切换时会触发那些钩子函数，加了keep-alive后呢？
假设动态组件component使用is控制组件显示，默认显示为A组件，可以切换到B组件。问：**首次进入页面以及is切换组件时，会触发A/B组件的哪些钩子函数，加了keep-alive后呢？

如果不加keep-alive，和正常的页面跳转进入新页面时，触发的钩子函数一致。

如果加了keep-alive，组件首次加载才会触发 created,mounted等钩子函数，切换时就不会触发created,mounted,beforedestroyed等，所以额外加一个activated和deactivated来控制切换时触发的钩子

#### component动态组件与使用v-if控制组件显示有什么区别
component动态组件可以理解为它就是v-if控制组件显示的语法糖。component切换组价时，和新进入页面时触发的钩子函数一致。

注意如果component加了keep-alive后，就会缓存页面了

#### element tabs组件切换页面时，组件会触发那些钩子函数
这里主要是要搞清除el-tabs是怎么实现组件切换的，使用el-tab-pane slot何不用这个插槽触发的钩子函数会相同吗？

如果不使用el-tab-pane的slot，只使用tab的导航，下面的内容自己控制，就看自己怎么控制下面的内容刷新了

如果内容放到了el-tab-pane的slot就复杂了，多曾嵌套slot，加上内置的lazy延迟加载，就需要好好写demo研究了

#### avtivated和created同时触发的问题


### 2020/06/11 周四
#### 改变窗口大小后其他tab页echart图表显示异常的问题
在echarts图表组件里，会监听window的resize事件，当窗口大小改变后，重绘图表。在单个页面里没有问题，但在tab组件切换时，各个tab页是懒加载(keep-alive)的状态，页面tab切换时，组件没有触发beforeDesotry事件。dom被隐藏后，window的resize事件没有被移除。导致在其他页面调整窗口大小，

除非每次切tab都会刷新，才不会有问题。最后想了个方法，每次点击tab手动触发window的resize事件进行重绘
...
之前一直觉得单元测试用处不是很大，通过今天的这个事情，让我对单元测试的用处有了深刻的认识，我后面在写代码时会尽量写对应的单元测试。
