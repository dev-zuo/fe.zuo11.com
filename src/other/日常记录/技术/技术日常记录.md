...
### 2020/07/30 周四
#### 什么是好的代码？
在web前端方面，什么是好的代码？我认为好的代码应该包含以下两个特性
- 高性能，低时延（性能优化）
...
技术始终是为业务需求服务的。基础建设是很重要的一个环节，这样有利于快速迭代开发，提高市场竞争力。有意去培养一支优秀的开发团队是一项很艰巨的任务。

#### vue使用js显示弹窗组件
其实核心就是使用vue的render函数
...
万元输入框怎么限制只能输入最多保留两位小数点的number类型数据
#### vscode插件开发，每次修改vue.config.js需要手动停止在运行
#### el-autocomplete debounce防抖默认300豪秒怎么实现的
#### loadash节流与防抖理解与应用
#### log2n对数在前端的应用场景：把文件大小或金额自动添加合适的单位

#### 关于常用组件、样式、工具函数封装代码快速复用的思考
怎么让搬砖更有效率
- 个人方面，形成自己的常用工具库utils、搜集各种场景常用的代码片段，快速ctrl+c、ctrl+v
- 团队方面，常用的工具函数、UI样式效果等总结归档，ctrl+c、ctrl+v
