...
之前有简单说明encodeURIComponent与encodeURI的区别：
...
Set k to k + 1.
Set R to the string-concatenation of the previous value of R and C.
Else,
Let cp be ! CodePointAt(string, k).
If cp.[[IsUnpairedSurrogate]] is true, throw a URIError exception.
...
For each element octet of Octets in List order, do
...
在总结某个知识点的过程中，我们可以发现介绍js相关知识点的平台很多，我们可以大致的判断出文档的详细程度如下：
...
- [js高程3的引用类型 - 单体内置对象 - global对象 - URI编码方法笔记]()
