...
两者的区别: **v-model主要用于表单输入的双向绑定，注重值的改变，.sync主要用于状态的切换**
...
我们一般在写程序时，很难发现自己逻辑上的bug，假设我们这里有写测试用例，考虑了很多种情况，那就可以避免这个问题。但现实是，我们目前大部分人都没有这个习惯，只要测试过了，就基本以为没问题了。但对于那种测试都测试不出来的在特定时间才会触发的bug，真的很可能造成线上bug，完全依赖于个人写代码时的逻辑严谨性。

怎么让自己写的代码更严谨，出问题的几率更小呢？那就是写测试用例。这样我们会考虑更多的特殊场景、而不是靠人肉测试。

之前一直觉得测试用例用处不是很大，通过今天的这个事情，让我对测试用例的用处有了深刻的认识，我后面在写代码时会尽量写对应的测试用例。
