...
这里我们需要在每次重新渲染时，先调用该echarts实例的resize()方法

另外在做echarts时，对于自适应缩放的图表，一定要注意窗口缩放时，重新调用resize()

#### http请求有哪几种传参方式
在swagger文档里，有一个传参类型的描述 Parameter Type，一般有四种
- header 放到请求头，也就是首部 headers 里
- path  放到url路径里比如 /user/123/ 这里 123是用户id
- query 查询参数，就是url后面 ? 符号之后的传参，一般用于get请求传参，比如 /user/123?a=xx&b=xx
- body 参数放到请求体，一般用于post请求，安全性较好

#### Object.assgin时是否会忽略null,undefined,空字符串
一般我们在需要设置某个对象的多个值时Object.assgin是一种很好的方法，但又怕当某个属性的值为空、null或undefined时，会自动跳过的情况。这里来做一个简单的测试

object.assign(a, {})

遍历对象的方法中，对空字符串、null、undefined的处理
...
#### element合并单元格利用css高度自定义表格border
### 2020/07/12 周日
