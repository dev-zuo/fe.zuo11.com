...
### 2020/06/11 周四
#### tab组件切换后echart图表渲染异常的问题
在echarts组件里，会监听window的resize事件，当窗口大小改变后，需要重新绘制图表。在单个页面里没有问题，但在tab组件切换时，各个tab页是懒加载的状态，某个tab显示或隐藏后，无法触发组件的 beforeDesotry事件。导致window的resize事件还在监听，而其dom已移除，这时候重绘echarts会导致宽高异常。
