...
npm包前面加@，代表scopes相关的包，可以理解为作用域(范围)包，**作用域包使我们可以创建与其他用户或组织创建的包同名的包，而不会发生冲突。**

A scope allows you to create a package with the same name as a package created by another user or Org without conflict.

当在package.json文件中被列为依赖项时，带作用域的程序包将在其作用域名称之前。作用域名称是介于@和斜线之间的所有内容：
...
注意：必须先注册一个npm用户帐户，然后才能发布用户范围的npm软件包。此外，要发布组织范围的软件包，您必须创建一个npm用户帐户，然后创建一个npm Org(组织)。
